...
public class ExternalBankTransferLedgerService {

    private static final String CLEARING_ACCOUNT =
    ...
    public void recordReservation(
        ...
        LedgerAccount clearingAccount = clearingAccount();
        validateCurrency(walletAccount, wallet.getCurrency());
        validateCurrency(clearingAccount, wallet.getCurrency());

        LedgerTransaction journal = ledgerTransactionRepository.save(
                LedgerTransaction.builder()
                        .reference(reference)
                        .eventType(LedgerEventType.EXTERNAL_BANK_TRANSFER)
    ...
    public void recordReversal(
        ...
        LedgerTransaction original = ledgerTransactionRepository
                ...
                .orElseThrow(() -> new IllegalStateException(
                        "Original external transfer ledger journal is missing"
                ));
    ...
    private LedgerAccount clearingAccount() {
        return ledgerAccountRepository.findByCode(CLEARING_ACCOUNT)
                .orElseThrow(() -> new IllegalStateException(
                        "External transfer clearing ledger account is missing"
                ));
    }

    private BigDecimal normalizeAmount(BigDecimal amount) {
        if (amount == null || amount.compareTo(BigDecimal.ZERO) <= 0) {
            throw new IllegalArgumentException("Transfer reservation amount must be greater than zero");
        }
