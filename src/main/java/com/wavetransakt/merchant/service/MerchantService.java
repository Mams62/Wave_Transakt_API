...
public class MerchantService {
    ...
    @Transactional
    public MerchantDtos.MerchantResponse createMerchant(UUID ownerUserId, MerchantDtos.CreateMerchantRequest request) {
    ...
    public List<MerchantDtos.MerchantResponse> getOwnedMerchants(UUID ownerUserId) {
        ...
        return merchantRepository.findAllByOwnerIdOrderByCreatedAtDesc(ownerUserId).stream().map(this::toMerchantResponse).toList();
    }
    ...
    public MerchantDtos.MerchantQrResponse getMerchantQr(UUID ownerUserId, UUID merchantId) {
        ...
        return new MerchantDtos.MerchantQrResponse(merchant.getId(), qrAddress.getPublicId(), MERCHANT_QR_PREFIX + qrAddress.getPublicId(), qrAddress.isActive());
    }
    ...
    public MerchantDtos.MerchantQrResolveResponse resolveMerchantQr(String payloadOrPublicId) {
        ...
        return new MerchantDtos.MerchantQrResolveResponse(merchant.getId(), merchant.getMerchantCode(), merchant.getBusinessName(), merchant.getBusinessType(), merchant.getStatus().name(), payable);
    }
    ...
    public List<MerchantDtos.PosTerminalResponse> getTerminals(UUID ownerUserId, UUID merchantId) {
        Merchant merchant = requireOwnedMerchant(ownerUserId, merchantId);
        return posTerminalRepository.findAllByMerchantIdOrderByCreatedAtDesc(merchant.getId()).stream().map(this::toTerminalResponse).toList();
    }

    @Transactional(readOnly = true)
    public List<MerchantDtos.MerchantPaymentResponse> getPayments(UUID ownerUserId, UUID merchantId) {
        Merchant merchant = requireOwnedMerchant(ownerUserId, merchantId);
        return merchantPaymentRepository.findAllByMerchantIdOrderByCreatedAtDesc(merchant.getId()).stream().map(this::toPaymentResponse).toList();
    }

    @Transactional(readOnly = true)
    public List<MerchantDtos.SettlementBatchResponse> getSettlementBatches(UUID ownerUserId, UUID merchantId) {
        Merchant merchant = requireOwnedMerchant(ownerUserId, merchantId);
        return merchantSettlementBatchRepository.findAllByMerchantIdOrderByCreatedAtDesc(merchant.getId()).stream().map(this::toSettlementBatchResponse).toList();
    }

    @Transactional(readOnly = true)
    public List<MerchantDtos.ReconciliationItemResponse> getReconciliationItems(UUID ownerUserId, UUID merchantId, UUID settlementBatchId) {
        Merchant merchant = requireOwnedMerchant(ownerUserId, merchantId);
        MerchantSettlementBatch batch = merchantSettlementBatchRepository.findByIdAndMerchantId(settlementBatchId, merchant.getId())
                .orElseThrow(() -> new IllegalArgumentException("Settlement batch not found"));
        return merchantReconciliationItemRepository.findAllBySettlementBatchIdOrderByCreatedAtAsc(batch.getId()).stream().map(this::toReconciliationItemResponse).toList();
    }
    ...
    private Merchant requireOwnedMerchant(UUID ownerUserId, UUID merchantId) {
        if (ownerUserId == null || merchantId == null) throw new IllegalArgumentException("Merchant and owner are required");
        return merchantRepository.findByIdAndOwnerId(merchantId, ownerUserId).orElseThrow(() -> new IllegalArgumentException("Merchant not found"));
    }

    private MerchantDtos.MerchantResponse toMerchantResponse(Merchant merchant) {
        boolean providerLinked = hasText(merchant.getProviderCode()) && hasText(merchant.getProviderMerchantId());
        return new MerchantDtos.MerchantResponse(merchant.getId(), merchant.getMerchantCode(), merchant.getBusinessName(), merchant.getBusinessType(), merchant.getStatus().name(), merchant.getProviderCode(), providerLinked, merchant.getCreatedAt());
    }
    ...
    private MerchantDtos.PosTerminalResponse toTerminalResponse(PosTerminal terminal) {
        ...
        return new MerchantDtos.PosTerminalResponse(
                terminal.getId(), terminal.getTerminalCode(), terminal.getStatus().name(), terminal.getProviderCode(),
                providerLinked, terminal.isSupportsQr(), terminal.isSupportsCard(), terminal.isSupportsNfc(), terminal.getCreatedAt()
        );
    }

    private MerchantDtos.MerchantPaymentResponse toPaymentResponse(MerchantPayment payment) {
        return new MerchantDtos.MerchantPaymentResponse(payment.getId(), payment.getPaymentReference(), payment.getChannel().name(), payment.getAmount(), payment.getCurrency(), payment.getStatus().name(), payment.getReceiptNumber(), payment.getSettlementStatus().name(), payment.getProviderCode(), payment.getCreatedAt());
    }

    private MerchantDtos.SettlementBatchResponse toSettlementBatchResponse(MerchantSettlementBatch batch) {
        return new MerchantDtos.SettlementBatchResponse(batch.getId(), batch.getProviderCode(), batch.getCurrency(), batch.getGrossAmount(), batch.getFeeAmount(), batch.getNetAmount(), batch.getPaymentCount(), batch.getStatus(), batch.getSettlementDate(), batch.getCreatedAt());
    }
    ...
    private MerchantDtos.ReconciliationItemResponse toReconciliationItemResponse(MerchantReconciliationItem item) {
        ...
        return new MerchantDtos.ReconciliationItemResponse(item.getId(), payment.getId(), payment.getPaymentReference(), item.getExpectedAmount(), item.getProviderAmount(), item.getDifferenceAmount(), item.getStatus(), item.getReason(), item.getCreatedAt());
    }

    private String extractPublicId(String value) {
        String normalized = normalizeRequired(value, "Merchant QR payload");
        if (normalized.startsWith(MERCHANT_QR_PREFIX)) normalized = normalized.substring(MERCHANT_QR_PREFIX.length());
        if (normalized.isBlank() || normalized.length() > 80) throw new IllegalArgumentException("Merchant QR is invalid");
        return normalized;
    }
    ...
    private String newMerchantCode() { return "WTM-" + compactUuid(12); }
    private String newMerchantQrPublicId() { return "MQ-" + compactUuid(24); }
    private String newTerminalCode() { return "WTPOS-" + compactUuid(16); }
